...
void Heap::cambiar(Aspirante * & Arreglo, int pos_h, int pos_p){
	if(pos_h == 1){
		if(Arreglo[pos_p].getPuntaje()> Arreglo[pos_h].getPuntaje()){
			Aspirante temp = Arreglo[pos_h];
		...
		if(Arreglo[pos_h].getPuntaje()<Arreglo[pos_p].getPuntaje()){
			Aspirante temp = Arreglo[pos_p];
			Arreglo[pos_h] = Arreglo[pos_p];
			Arreglo[pos_p] = temp;
		}else{
			return;
